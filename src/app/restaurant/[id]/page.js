...
export default async function RestaurantMenu({ params }) {
  ...
  return <main className="page-shell menu-page">
    <header className={`menu-hero restaurant-cover ${bannerUrl?'has-banner':''}`}>{bannerUrl&&<Image className="restaurant-cover-image" src={bannerUrl} alt={`${restaurant?.name || 'Restaurant'} cover`} fill sizes="100vw" priority unoptimized referrerPolicy="no-referrer"/>}<div className="restaurant-cover-shade"/><Link href="/" className="back-link"><ArrowLeft size={19} /> All kitchens</Link><div className="restaurant-cover-copy"><p className="eyebrow">Freshly made for you</p><h1>{restaurant?.name || 'Restaurant menu'}</h1><p>{restaurant?.description || 'Choose a favorite, then make it exactly yours.'}</p></div></header>
    <section className="menu-panel">{error ? <div className="state-card"><ChefHat size={34} /><h3>We couldn&apos;t load this menu.</h3><p>Please head back and try again.</p></div> : !menuItems.length ? <div className="state-card"><ChefHat size={34} /><h3>Nothing on the pass yet.</h3><p>This kitchen is updating its menu.</p></div> : <MenuClient items={menuItems} restaurantId={id} />}</section>
  </main>;
}
