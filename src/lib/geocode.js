...
const cacheKey = (city, barangay) => `${normalize(barangay).toLowerCase()}|${normalize(city).toLowerCase()}`;

async function publicGeocode(city, barangay) {
  const task = runtime.__tmgGeocodeQueue.then(async () => {
    ...
    const query = `${barangay}, ${city}, Metro Manila, Philippines`;
...
export async function geocodeDeliveryArea(cityValue, barangayValue) {
  ...
  const key = cacheKey(city, barangay);
  ...
  const result = await publicGeocode(city, barangay);
