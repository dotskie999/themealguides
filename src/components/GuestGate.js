...
const initialForm = {
  market_code:'', country_code:'', city:'', city_code:'', barangay:'', house_number:'', landmark:'', digital_address:'',
...
export default function GuestGate({ children }) {
  ...
  const market = form.market_code ? getMarket(form.market_code) : null;
  ...
  const chooseMarket = (marketCode) => {
    const chosen=getMarket(marketCode);
    setForm((current)=>({...current,market_code:chosen.code,country_code:chosen.countryCode,contact_number:chosen.phonePrefix,city:'',city_code:'',barangay:'',latitude:null,longitude:null,location_source:null}));
  ...
  return <main className="guest-onboarding"><section className="guest-card"><Image src="/the-meal-guides-logo.png" alt="The Meal Guides" width={150} height={100} priority/>
    {step==='market'?<><span className="guest-step"><Globe2 size={15}/> Choose your market</span><h1>Where are you ordering from?</h1><p>Choose a delivery market so we can show the right kitchens, address options, phone format, and currency.</p><div className="market-picker">{MARKET_OPTIONS.map((option)=><button key={option.code} onClick={()=>chooseMarket(option.code)}><span>{option.countryCode==='PH'?'🇵🇭':'🇬🇭'}</span><div><strong>{option.country}</strong><small>{option.region}</small></div><ArrowRight/></button>)}</div></>:step==='location'?<>
      <span className="guest-step"><Navigation size={15}/> {market.label}</span><h1>Where should we guide the feast?</h1><p>Your doorstep is the coordinate, and your taste buds are the boss!</p><form onSubmit={confirmLocation} className="guest-form">
        ...
        <p className="address-location-note full"><MapPin size={17}/><span>Distance uses your selected delivery area—not your phone&apos;s live location. Your street and landmark are never sent for map lookup.</span></p>{error&&<p className="form-error full">{error}</p>}
        <div className="guest-form-actions full"><button type="button" className="secondary-button" onClick={()=>setStep('market')}>Change market</button><button className="primary-button" disabled={loadingLocations}>Find food near me <ArrowRight size={19}/></button></div>
