...
export default function RestaurantDistance({ restaurant }) {
  ...
  return <p className={`restaurant-location ${outside?'outside':''}`}><MapPin size={16}/><span><strong>{formatDistance(distance)} · {formatTravelEstimate(distance)} estimated</strong>{restaurant.address&&<small>{restaurant.address}</small>}{outside&&<em>Outside delivery area</em>}</span></p>;
}
